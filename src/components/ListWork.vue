...
<div id="conatiner">
        ...
        <div class="w-full mx-auto md:p-6 md:pb-2">
            ...
            <ul class="mx-4 my-3 text-left">
                <li v-if="ws_items.length === 0">この日の報告はありません</li>
                <li class="pb-1" v-for="(ws_item, index) in ws_items" :key="index">
                    <div class="shadow-md">
                        <div class="tab w-full overflow-hidden border-t">
                            <input @click="OpenList" class="absolute opacity-0" :id="['ws_tab-single-' + index]" type="radio" name="tabs">
                            <label class="md:flex justify-between w-full p-4 pb-2 leading-normal cursor-pointer" :for="['ws_tab-single-' + index]">
                                <div class="px-2 md:px-0">
                                    <h3 class="text-sm font-semibold"><span class="px-5 py-1 mr-3 text-xs border rounded-2xl text-green-500 border-green-500">勤怠</span>{{ ws_item[0] }}｜出勤打刻</h3>
        ...
        <div class="w-full mx-auto md:p-6 md:pb-2">
            ...
            <hr>
            <ul class="mx-4 my-3 text-left">
                <li v-if="we_items.length === 0">この日の報告はありません</li>
                <li class="pb-1" v-for="(we_item, index) in we_items" :key="index">
                    <div class="shadow-md">
                        <div class="tab w-full overflow-hidden border-t">
                            <input @click="OpenList" class="absolute opacity-0" :id="['we_tab-single-' + index]" type="radio" name="tabs">
                            <label class="md:flex justify-between w-full p-4 pb-2 leading-normal cursor-pointer" :for="['we_tab-single-' + index]">
                                <div class="px-2 md:px-0">
                                    <h3 class="text-sm font-semibold"><span class="px-5 py-1 mr-3 text-xs border rounded-2xl text-green-500 border-green-500">勤怠</span>{{ we_item[0] }}｜退勤打刻</h3>
...
export default {
    ...
    getReport: function() {
            // リスト初期化
            this.ws_items.splice(0, this.ws_items.length)
            this.we_items.splice(0, this.we_items.length)
                    ...
                    qs.forEach(function(doc) {
                        ...
                        .then(function(StartWork) {
                            StartWork.forEach(function(qs) {
                                ...
                                _wsItem.push(DateTime, qs.data().latitude, qs.data().longitude, qs.data().status)
                                self.ws_items.push(_wsItem);
                            })
                        })
                        .then(
                            // OCN業務開始
                            ...
                            .then(function(EndWork) {
                                EndWork.forEach(function(qs) {
                                    ...
                                    _weItem.push(DateTime, qs.data().latitude, qs.data().longitude, qs.data().status)
                                    self.we_items.push(_weItem);
                                })
                            })
                        )
                    });
